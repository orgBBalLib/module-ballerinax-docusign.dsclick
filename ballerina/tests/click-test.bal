...
function testCreateClickWrap() returns error? {
    ...
    json responseJson = check response.getJsonPayload();
    string? returnedClickwrapName = <string?>check responseJson.clickwrapName;
    test:assertEquals(returnedClickwrapName, clickwrapName);
    string? clickwrapId = <string?>check responseJson.clickwrapId;
    if clickwrapId is () {
...
function testDeleteClickWrap() returns error? {
    ...
    string? deletedClickwrapId = <string?>check deleteResponseJson.clickwrapId;
    test:assertEquals(deletedClickwrapId, clickwrapId);
}
...
function testGetSingleClickwrap() returns error? {
    ...
    string? returnedAccountId = <string?>check getResponseJson.accountId;
    test:assertEquals(returnedAccountId, accountId);
    _ = check docuSignClient->/v1/accounts/[accountId]/clickwraps/[clickwrapId].delete();
}
...
function testDeleteClickwrapVersionByNumber() returns error? {
    ...
    string? versionNumber = <string?>check responseJson.versionNumber;
    if versionNumber is () {
    ...
    json deleteResponseJson = check deleteResponse.getJsonPayload();
    string? deletedVersionNumber = <string?>check deleteResponseJson.versionNumber;
    test:assertEquals(deletedVersionNumber, versionNumber);
}
...
function testGetClickwrapAgreementsByVersionNumber() returns error? {
    ...
    json userAgreements = check agreementResponseJson.userAgreements;
...
function testUpdateClickwrapVersionByNumber() returns error? {
    ...
    string? updatedClickwrapName = <string?>check updateResJson.clickwrapName;
...
function testPostUserAgreement() returns error? {
   string base64Encoded = array:toBase64(check io:fileReadBytes("resources/README.txt"));
    ...
    http:Response response = check docuSignClient->/v1/accounts/[accountId]/clickwraps.post(payload);
    json responseJson = check response.getJsonPayload();
    string? status = <string?>check responseJson.status;
    if status == "active" {
        string? clickwrapId = <string?>check responseJson.clickwrapId;
        if clickwrapId is () {
            return error("Clickwrap Id is not available");
        ...
        json agreementResponseJson = check agreementResponse.getJsonPayload();
        string? returnedAccountId = <string?>check agreementResponseJson.accountId;
        test:assertEquals(returnedAccountId, accountId);
    }
}